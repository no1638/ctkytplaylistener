...
class App(CTk):
    ...
    def check_event(self, internalIndex):
        while not self.purge_flag and not self.exit_flag and self._currentIndex == internalIndex:
            #print("check event")
            # if self._currentIndex != internalIndex:
            #     print("[-] Current Index not equal to Internal Index.")
            #     break
    ...
    def loadNextFile(self):
        if self._loopToggle == "N" or self._loopToggle == "A":
            #handle condition N / A independently
            print(f"[/] Loop toggle is: '{self._loopToggle}'")
            print(f"[/] Shuffle Toggle: {self.shuffleToggle.get()}")
    ...
    def playlistHandler(self, promptURL=True, selectedTitle=None):
        ...
        if redownload or not os.path.exists(f"sd/playlists/{self._playlistTitle}"):
            ...
            for index, video in enumerate(self.currentPlaylist.video_urls):
                ...
                self.dwl_mp3(video, index, True, True)
        ...
        #         roster_titles = f.read().splitlines()

        #     # Sort the file names based on their numeric values
        #     file_names = [file for file in os.listdir(os.getcwd()) if file.endswith(".ogg")]
        #     file_names.sort(key=lambda x: int(x.split('.')[0]))

        #     # Create the currentQueue list
        #     #self.currentQueue = []
        #     for index, file in enumerate(file_names):
        #         self.currentQueue.append([roster_titles[index], f"sd/playlists/{self._playlistTitle}/{file}"])
        ...
        if os.path.exists(f"roster.txt"):
            ...
            for index, file in enumerate(os.listdir(os.getcwd())):
                if file.endswith(".ogg"):
                    print(file)
                    print(index)
                    selectedFile = os.listdir(os.getcwd())[index]
                    self.currentQueue.append([rosterTitles[index], f"sd/playlists/{self._playlistTitle}/{index}.ogg"])
        else:
            print("CALLED ELSE")
            for index, file in enumerate(os.listdir(os.getcwd())):
                if file.endswith(".ogg"):
                    selectedFile = os.listdir(os.getcwd())[index]
                    self.currentQueue.append([f"{file}", f"sd/playlists/{self._playlistTitle}/{index}.ogg"])
        #os.chdir(self._homeDirectory)
        print(self.currentQueue)
        #self.currentQueue = []
...
def on_closing():
    ...
    root._currentIndex = root._currentIndex + 1
    print("closing")
    try:
